...
if (!etherAmount) {
	throw new Error(
		"Please set FUEL_ETHER_AMOUNT in your environment variables.",
	);
}
if (!insuranceContract || !isAddress(insuranceContract)) {
	throw new Error(
		"Please set INSURANCE_CONTRACT_ADDRESS in your environment variables.",
	);
}
...
console.log(
	`Funded ${insuranceContract} with ${etherAmount} ETH. Transaction hash: ${txReceipt.transactionHash}`,
);
